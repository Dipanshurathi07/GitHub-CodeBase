...
export default function DiffTerminal() {
  ...
  return (
    <div className="w-full min-w-0 max-w-xl rounded-lg border border-ink-border bg-ink-panel shadow-glow overflow-hidden">
      ...
      <div className="min-w-0 p-3 sm:p-4 font-mono text-[11px] sm:text-[13px] leading-[1.85] min-h-[230px] sm:min-h-[280px] overflow-hidden sm:overflow-x-auto">
        {SEQUENCE.slice(0, visibleLines).map((line, i) => (
          <div
            key={`${cycle}-${i}`}
            className={
              'whitespace-pre-wrap break-words sm:whitespace-pre sm:break-normal animate-fadeUp ' +
              (line.type === 'add'
